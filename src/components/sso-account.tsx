...
const SsoAccountButton: React.FC<SsoAccountButtonProps> = ({
  ...
  return (
    <button
      ...
      className="group relative w-full flex items-center gap-4 rounded-xl border border-hairline dark:border-white/10 bg-white dark:bg-white/5 px-5 py-4 text-left transition-all duration-200 hover:border-brand-green/60 hover:shadow-sm hover:shadow-brand-green/10 disabled:opacity-90 disabled:cursor-not-allowed hover:cursor-pointer"
    >
      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-brand-green-soft dark:bg-brand-green/15 text-brand-green-dark dark:text-brand-green transition-colors group-hover:bg-brand-green/15 dark:group-hover:bg-brand-green/20">
        {icon}
      </span>
      <span className="flex flex-col gap-0.5 min-w-0">
        ...
        <span className="text-xs text-steel dark:text-white/50">{description}</span>
      </span>
      {isPending && <Spinner variant="spinner" size="sm" color="current"/>}
      <svg
        className="ml-auto w-4 h-4 text-muted dark:text-white/40 transition-transform group-hover:translate-x-0.5 group-hover:text-brand-green"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
